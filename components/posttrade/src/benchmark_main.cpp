// The measurement harness for post-trade: the cost per booked event, ownership, the tape and
// both accounts included, timed from event bytes in to the books moved, warm-up excluded, raw
// series and manifest as everywhere (P-14). The ledger sits off the latency path by design; the
// number exists so that claim is a measurement rather than a hope.
//
//   posttrade-benchmark --results DIR [--events N] [--warmup N] [--core N] [--label L]
...
int main(const int count, char** values) {
  ...
  const std::string label = argument(count, values, "--label", "posttrade");
  ...
  if (resultsPath.empty() || events <= warmup) {
    std::fprintf(stderr,
                 "usage: posttrade-benchmark --results DIR [--events N] [--warmup N] [--core N]"
                 " [--label L]\n");
    return 2;
  }
  ...
  uname(&machine);
  {
    ...
    manifest << "{\n"
             << "  \"label\": \"" << label << "\",\n"
             << "  \"path\": \"one event booked: ownership, the tape and both accounts\",\n"
             << "  \"events\": " << events << ",\n"
             << "  \"warmup\": " << warmup << ",\n"
             << "  \"isolation\": \"" << isolation << "\",\n"
