...
#ifndef _EXCHANGE_PROTOCOL_COMMANDCONTEXT_CXX_H_
#define _EXCHANGE_PROTOCOL_COMMANDCONTEXT_CXX_H_
...
public:
    ...
    {
        if (SBE_BOUNDS_CHECK_EXPECT(((m_offset + 24) > m_bufferLength), false))
        {
    ...
    CommandContext(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        CommandContext(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    CommandContext(
        char *buffer,
        const std::uint64_t bufferLength) :
        CommandContext(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }
    ...
    CommandContext &sequence(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    CommandContext &timestamp(const std::uint64_t value) SBE_NOEXCEPT
    {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    CommandContext &instrumentId(const std::uint32_t value) SBE_NOEXCEPT
    {
    ...
    CommandContext &reserved(const std::uint32_t value) SBE_NOEXCEPT
    {
        std::uint32_t val = SBE_LITTLE_ENDIAN_ENCODE_32(value);
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, CommandContext &writer)
{
    builder << '{';
    builder << R"("sequence": )";
