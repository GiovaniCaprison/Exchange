...
#ifndef _EXCHANGE_PROTOCOL_SESSIONSTATECHANGED_CXX_H_
#define _EXCHANGE_PROTOCOL_SESSIONSTATECHANGED_CXX_H_
...
public:
    ...
    SessionStateChanged(char *buffer, const std::uint64_t bufferLength) :
        SessionStateChanged(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    SessionStateChanged(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        SessionStateChanged(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    SessionStateChanged &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    SessionStateChanged &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    SessionStateChanged &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    {
        SessionStateChanged skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
...
public:
    ...
    SBE_NODISCARD std::uint8_t stateRaw() const SBE_NOEXCEPT
    {
        std::uint8_t val;
    ...
    SBE_NODISCARD SessionState::Value state() const
    {
        std::uint8_t val;
        std::memcpy(&val, m_buffer + m_offset + 32, sizeof(std::uint8_t));
        return SessionState::get((val));
    }

    SessionStateChanged &state(const SessionState::Value value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const SessionStateChanged &_writer)
{
    SessionStateChanged writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "SessionStateChanged", )";
