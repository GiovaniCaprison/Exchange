...
#ifndef _EXCHANGE_PROTOCOL_REPLACEORDER_CXX_H_
#define _EXCHANGE_PROTOCOL_REPLACEORDER_CXX_H_
...
public:
    ...
    ReplaceOrder(char *buffer, const std::uint64_t bufferLength) :
        ReplaceOrder(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    ReplaceOrder(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        ReplaceOrder(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    ReplaceOrder &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    ReplaceOrder &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        ReplaceOrder skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
...
public:
    ...
    ReplaceOrder &clientOrderId(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    ReplaceOrder &quantity(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    ReplaceOrder &price(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    ReplaceOrder &participantId(const std::uint32_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const ReplaceOrder &_writer)
{
    ReplaceOrder writer(
        _writer.m_buffer,
        _writer.m_offset,
