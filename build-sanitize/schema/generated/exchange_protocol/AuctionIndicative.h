...
#ifndef _EXCHANGE_PROTOCOL_AUCTIONINDICATIVE_CXX_H_
#define _EXCHANGE_PROTOCOL_AUCTIONINDICATIVE_CXX_H_
...
public:
    ...
    AuctionIndicative(char *buffer, const std::uint64_t bufferLength) :
        AuctionIndicative(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    AuctionIndicative(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        AuctionIndicative(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    AuctionIndicative &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    AuctionIndicative &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    AuctionIndicative &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    {
        AuctionIndicative skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
...
public:
    ...
    AuctionIndicative &price(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    AuctionIndicative &quantity(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const AuctionIndicative &_writer)
{
    AuctionIndicative writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "AuctionIndicative", )";
