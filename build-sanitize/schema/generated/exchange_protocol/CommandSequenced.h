...
#ifndef _EXCHANGE_PROTOCOL_COMMANDSEQUENCED_CXX_H_
#define _EXCHANGE_PROTOCOL_COMMANDSEQUENCED_CXX_H_
...
public:
    ...
    CommandSequenced(char *buffer, const std::uint64_t bufferLength) :
        CommandSequenced(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    CommandSequenced(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        CommandSequenced(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    CommandSequenced &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    CommandSequenced &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    CommandSequenced &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    {
        CommandSequenced skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    CommandSequenced &gatewaySequence(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    CommandSequenced &sequence(const std::uint64_t value) SBE_NOEXCEPT
    {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    CommandSequenced &timestamp(const std::uint64_t value) SBE_NOEXCEPT
    {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const CommandSequenced &_writer)
{
    CommandSequenced writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "CommandSequenced", )";
