...
#ifndef _EXCHANGE_PROTOCOL_ORDERFLAGS_CXX_H_
#define _EXCHANGE_PROTOCOL_ORDERFLAGS_CXX_H_
...
public:
    ...
    OrderFlags(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        OrderFlags(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    OrderFlags(
        char *buffer,
        const std::uint64_t bufferLength) :
        OrderFlags(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }
    ...
    OrderFlags &rawValue(std::uint8_t value)
    {
        std::memcpy(m_buffer + m_offset, &value, sizeof(std::uint8_t));
        return *this;
    }

    static bool postOnly(const std::uint8_t bits)
    {
        return (bits & (static_cast<std::uint8_t>(1) << 0u)) != 0;
    }

    static std::uint8_t postOnly(const std::uint8_t bits, const bool value)
    {
    ...
    SBE_NODISCARD bool postOnly() const
    {
        std::uint8_t val;
        std::memcpy(&val, m_buffer + m_offset, sizeof(std::uint8_t));
    ...
    OrderFlags &postOnly(const bool value)
    {
        std::uint8_t bits;
    ...
    friend std::basic_ostream<CharT, Traits> & operator << (
        std::basic_ostream<CharT, Traits> &builder, OrderFlags &writer)
    {
        builder << '[';
        if (writer.postOnly())
        {
