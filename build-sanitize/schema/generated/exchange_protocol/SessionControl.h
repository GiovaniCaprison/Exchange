...
#ifndef _EXCHANGE_PROTOCOL_SESSIONCONTROL_CXX_H_
#define _EXCHANGE_PROTOCOL_SESSIONCONTROL_CXX_H_
...
public:
    ...
    SessionControl(char *buffer, const std::uint64_t bufferLength) :
        SessionControl(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    SessionControl(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        SessionControl(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    SessionControl &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    SessionControl &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        SessionControl skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
...
public:
    ...
    SessionControl &state(const SessionState::Value value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const SessionControl &_writer)
{
    SessionControl writer(
        _writer.m_buffer,
        _writer.m_offset,
