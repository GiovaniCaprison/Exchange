...
#ifndef _EXCHANGE_PROTOCOL_ORDERREDUCED_CXX_H_
#define _EXCHANGE_PROTOCOL_ORDERREDUCED_CXX_H_
...
public:
    ...
    OrderReduced(char *buffer, const std::uint64_t bufferLength) :
        OrderReduced(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    OrderReduced(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        OrderReduced(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    OrderReduced &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    OrderReduced &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        OrderReduced skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
...
public:
    ...
    OrderReduced &orderId(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    OrderReduced &quantity(const std::int64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const OrderReduced &_writer)
{
    OrderReduced writer(
        _writer.m_buffer,
        _writer.m_offset,
