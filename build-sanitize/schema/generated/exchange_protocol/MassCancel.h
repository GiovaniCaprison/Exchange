...
#ifndef _EXCHANGE_PROTOCOL_MASSCANCEL_CXX_H_
#define _EXCHANGE_PROTOCOL_MASSCANCEL_CXX_H_
...
public:
    ...
    MassCancel(char *buffer, const std::uint64_t bufferLength) :
        MassCancel(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    MassCancel(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        MassCancel(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    MassCancel &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    MassCancel &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        MassCancel skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
...
public:
    ...
    MassCancel &clientOrderId(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    MassCancel &participantId(const std::uint32_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const MassCancel &_writer)
{
    MassCancel writer(
        _writer.m_buffer,
        _writer.m_offset,
