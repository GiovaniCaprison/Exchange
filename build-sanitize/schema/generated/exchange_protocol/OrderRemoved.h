...
#ifndef _EXCHANGE_PROTOCOL_ORDERREMOVED_CXX_H_
#define _EXCHANGE_PROTOCOL_ORDERREMOVED_CXX_H_
...
public:
    static constexpr std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(49);
    ...
    OrderRemoved(char *buffer, const std::uint64_t bufferLength) :
        OrderRemoved(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    OrderRemoved(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        OrderRemoved(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    OrderRemoved &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    OrderRemoved &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        OrderRemoved skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
...
public:
    ...
    OrderRemoved &orderId(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    OrderRemoved &quantity(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    SBE_NODISCARD RemoveReason::Value reason() const
    {
        std::uint8_t val;
        std::memcpy(&val, m_buffer + m_offset + 48, sizeof(std::uint8_t));
        return RemoveReason::get((val));
    }

    OrderRemoved &reason(const RemoveReason::Value value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const OrderRemoved &_writer)
{
    OrderRemoved writer(
        _writer.m_buffer,
        _writer.m_offset,
