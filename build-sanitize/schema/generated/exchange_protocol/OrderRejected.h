...
#ifndef _EXCHANGE_PROTOCOL_ORDERREJECTED_CXX_H_
#define _EXCHANGE_PROTOCOL_ORDERREJECTED_CXX_H_
...
public:
    static constexpr std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(45);
    ...
    OrderRejected(char *buffer, const std::uint64_t bufferLength) :
        OrderRejected(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    OrderRejected(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        OrderRejected(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint16_t sbeBlockLength() SBE_NOEXCEPT
    {
        return static_cast<std::uint16_t>(45);
    }
    ...
    OrderRejected &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    OrderRejected &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        OrderRejected skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
...
public:
    ...
    OrderRejected &clientOrderId(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    OrderRejected &participantId(const std::uint32_t value) SBE_NOEXCEPT
    {
    ...
    SBE_NODISCARD static SBE_CONSTEXPR std::size_t reasonEncodingLength() SBE_NOEXCEPT
    {
        return 1;
    }

    SBE_NODISCARD std::uint8_t reasonRaw() const SBE_NOEXCEPT
    {
        std::uint8_t val;
    ...
    SBE_NODISCARD RejectReason::Value reason() const
    {
        std::uint8_t val;
        std::memcpy(&val, m_buffer + m_offset + 44, sizeof(std::uint8_t));
    ...
    OrderRejected &reason(const RejectReason::Value value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const OrderRejected &_writer)
{
    OrderRejected writer(
        _writer.m_buffer,
        _writer.m_offset,
