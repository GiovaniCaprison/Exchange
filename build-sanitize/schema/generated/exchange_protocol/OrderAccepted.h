...
#ifndef _EXCHANGE_PROTOCOL_ORDERACCEPTED_CXX_H_
#define _EXCHANGE_PROTOCOL_ORDERACCEPTED_CXX_H_
...
public:
    ...
    OrderAccepted(char *buffer, const std::uint64_t bufferLength) :
        OrderAccepted(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    OrderAccepted(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        OrderAccepted(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    OrderAccepted &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    OrderAccepted &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        OrderAccepted skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
...
public:
    ...
    OrderAccepted &orderId(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    OrderAccepted &clientOrderId(const std::uint64_t value) SBE_NOEXCEPT
    {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    OrderAccepted &participantId(const std::uint32_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const OrderAccepted &_writer)
{
    OrderAccepted writer(
        _writer.m_buffer,
        _writer.m_offset,
