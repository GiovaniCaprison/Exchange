...
#ifndef _EXCHANGE_PROTOCOL_ORDERRESTED_CXX_H_
#define _EXCHANGE_PROTOCOL_ORDERRESTED_CXX_H_
...
public:
    ...
    OrderRested(char *buffer, const std::uint64_t bufferLength) :
        OrderRested(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    OrderRested(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        OrderRested(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    OrderRested &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    OrderRested &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        OrderRested skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
...
public:
    ...
    OrderRested &orderId(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    OrderRested &price(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    OrderRested &quantity(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    OrderRested &side(const Side::Value value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const OrderRested &_writer)
{
    OrderRested writer(
        _writer.m_buffer,
        _writer.m_offset,
