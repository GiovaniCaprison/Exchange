...
#ifndef _EXCHANGE_PROTOCOL_ORDERTRIGGERED_CXX_H_
#define _EXCHANGE_PROTOCOL_ORDERTRIGGERED_CXX_H_
...
public:
    ...
    OrderTriggered(char *buffer, const std::uint64_t bufferLength) :
        OrderTriggered(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    OrderTriggered(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        OrderTriggered(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    OrderTriggered &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    OrderTriggered &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        OrderTriggered skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
...
public:
    ...
    OrderTriggered &orderId(const std::uint64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const OrderTriggered &_writer)
{
    OrderTriggered writer(
        _writer.m_buffer,
        _writer.m_offset,
